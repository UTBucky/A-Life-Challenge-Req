...
class GridViewer:
    ...
    def __init__(self, environment, tile_size=10, sidebar_width=200):
        """
        Stores an a-life environment to render, tilesize and sidebar are default 10/200 respectively
        """
        ...
        self.window_size = (environment.get_size() * tile_size + sidebar_width, environment.get_size() * tile_size)
        pygame.init()
    ...
    def draw(self):
        ...
        # Draw the screen
        pygame.draw.rect(self.screen, (30, 30, 30), pygame.Rect(0, 0, sw, size * ts))

        # Generation counter
        gen_text = self.font.render(f"Generation: {self.env.get_generation()}", True, (255, 255, 255))
